...
from app.api.deps import AiDep, CurrentUser, PrimaryClient, SecondaryClient
...
async def request_ai_check(
    ...
    return await ai_check_service.request_check(
        primary, secondary, ai, user, payload.bando_slug
    )
...
async def list_ai_checks(
    ...
    return await ai_check_service.list_checks(primary, user, bando_slug, page, page_size)
...
async def get_ai_check(
    check_id: str, user: CurrentUser, primary: PrimaryClient
) -> AiCheckOut:
    return await ai_check_service.get_check(primary, user, check_id)
