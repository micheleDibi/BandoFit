...
async def save_bando(
    ...
    """Salva un bando tra i preferiti dell'azienda attiva (idempotente: già
    salvato → lo ritorna)."""
...
async def saved_ids(
    ...
    return await saved_bandi_service.saved_ids(primary, user["id"], active)
...
async def list_saved(
    ...
    return await saved_bandi_service.list_saved(
        primary, secondary, user["id"], active, page, page_size
    )
...
async def remove_bando(
    ...
    """Rimuove dai preferiti dell'azienda attiva (idempotente; l'eventuale
    evento in calendario resta)."""
    await saved_bandi_service.remove_bando(primary, user["id"], active, bando_id)
