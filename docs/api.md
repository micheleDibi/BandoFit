...
### `GET /me/entitlements`
**Le quote dell'account in un'unica risposta** (migration 0030) — la fonte da cui il frontend legge i limiti, senza mai ricalcolarli: `{editable, seats, companies, ai_checks}` dove ogni risorsa è `{base, extra, effettivo, usato, residuo}` (per `ai_checks` anche `periodo_inizio`/`periodo_fine`, la finestra del ciclo attivo). `base` viene dal piano attivo del titolare (per `companies` include l'eventuale override admin), `extra` dalle unità di **addon allocativi** possedute (`addons.risorsa`), `effettivo` dalla formula unica SQL (`fn_entitlement_detail`, dormienza inclusa: l'extra conta solo se la base abilita la capability). Per un **collegato attivo** lo snapshot è quello del **titolare** (pool condiviso) con `editable: false` e, su `ai_checks`, i campi PROPRI del membro (WP6): `budget_membro` (null = illimitato) e `usati_membro` (consumi del ciclo); per i titolari sono null. Gli stessi numeri alimentano `family.limit` di `GET /me`, `GET /me/family` e la quota di `GET /me/ai-checks`: nessuna divergenza possibile tra display e arbitri (che applicano lo stesso effettivo via `fn_family_limit`/`fn_effective_max_aziende`).
...
Body `{ "bando_slug": "..." }`. Avvia l'analisi e risponde subito con la riga `pending`: la generazione gira in background (1–2 minuti) e si segue con la GET (polling). **Dal WP6 (0031) anche un membro ATTIVO può avviarla** — sull'azienda attiva (che il resolver garantisce ∈ alla sua visibilità) ed entro il **budget assegnato dal titolare** (`family_members.ai_check_budget`, null = illimitato): il vincolo del singolo check è **min(residuo membro, residuo pool)**, verificato sotto lo stesso lock owner del pool (due membri concorrenti si serializzano; l'insert della riga È il decremento atomico; un errore non consuma — rollback naturale; il doppio submit è respinto dall'indice one-pending). I guasti del provider AI (timeout compreso: esito ignoto, nessun retry automatico) non arrivano mai come errore HTTP di questa POST — emergono come `status: "error"` con `error_detail` sulla riga letta via GET. Un'analisi `pending` da oltre 10 minuti viene chiusa come `error` alla lettura/POST successiva (failsafe per i riavvii). Audit per consumo: riga `ai_checks` (chi = `user_id`, quando, quale bando, quale azienda, esito) + `audit_log` `company.ai_check_requested` (con `company_profile_id` e richiedente) + registro spesa `api_usage_events`.
Errori: `503 ai_not_configured`, `403 forbidden` (membro con **budget esaurito**, o editable=False senza membership attiva), `400 bad_request` (dati aziendali insufficienti), `404 not_found` (bando), `409 ai_check_in_progress` (analisi già in corso o altra operazione sull'azienda), `429 ai_quota_exceeded` (pool del titolare esaurito), `429 ai_check_cooldown` (5 min per coppia azienda×bando).
