...
**Landing pubblica** (`pages/Landing.tsx`): pagina marketing per l'utente sloggato (redirect a `/app/bandi` se c'è sessione). Sezioni: header sticky con ancore (Funzionalità · Come funziona · Piani · FAQ, smooth-scroll) + Accedi/Registrati; hero a due colonne (gradiente firma `from-brand-950 via-brand-900 to-brand-700`) con `HeroShowcase` (mock di prodotto *illustrativo* costruito dai token — scheda bando + widget AI-check con anello punteggio via `lib/scoreColor.ts`, `aria-hidden`); problema→soluzione; griglia funzionalità con l'**AI-check in evidenza** (`FeatureCard` variante `featured`); «come funziona» in 4 passi; «perché BandoFit» + fascia statistiche (nessuna statistica di utenti inventata; il **conteggio bandi è un claim di marketing**, non un dato: vive in `lib/copy.ts` ed è ripetuto in hero, FAQ e fascia — da lì, per non divergere); piani (riuso di `usePlans`+`PlanCard`, gestione `su_richiesta` invariata); FAQ (`Faq`, accordion `<details>` nativo); CTA finale; footer ricco (ancore, Accedi/Registrati, `PoweredBy`). Copy onesto: gli **alert email non sono implementati** → il tema scadenze punta sul Calendario reale e gli alert sono citati come «in arrivo» sotto i piani. Componenti presentazionali co-locati in `components/landing/` (`HeroShowcase`, `SectionHeading`, `FeatureCard`, `Faq`), solo token, nessuna logica di rete.
...
- Le stringhe user-facing **ripetute in più punti** (per ora: il claim sui bandi della landing, in hero, FAQ e fascia statistiche) stanno in **`lib/copy.ts`**. Non è i18n: è l'antidoto alla divergenza. Le stringhe usate una volta sola restano inline nel componente, come nel resto dell'app.
