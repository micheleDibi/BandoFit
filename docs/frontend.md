...
- **Pagina «Calendario»** (`/app/calendario`, mese nell'URL `?m=YYYY-MM`): un'unica Card con **toolbar integrata** (mese + frecce + «Oggi» + legenda), griglia mensile nativa (lunedì per primo, 6 settimane fisse, bordi interni sottili, numero in alto a sinistra, hint «+» al passaggio del mouse, `Intl it-IT` — nessuna libreria di date) e **agenda del giorno come pannello incorporato** a destra su desktop (`xl:w-80`, sfondo attenuato, riempie l'altezza; sotto la griglia su mobile). Interazione diretta: **click su un giorno → si apre subito il form di creazione** con la data precompilata (e il giorno si seleziona); **click su un chip evento → dialog di modifica**. Niente interattivi annidati: la cella è un div con un bottone di sfondo (`absolute inset-0`) e i chip come bottoni FRATELLI sovrapposti (`z-10`); su mobile i chip sono pallini presentazionali e gli eventi si aprono dall'agenda. `EventDialog`: titolo, data (`type="date"`), «Tutto il giorno», orari (`type="time"`), note (`TextareaField`, nuovo componente ui); per gli eventi bando la data è una riga fissa («deriva dal bando ufficiale») con link al bando; eliminazione con conferma a due passi. Hook `useCalendarEvents(anno, mese)` (key `["calendar", anno, mese]`); le mutazioni invalidano il prefisso `["calendar"]`; `useAddBandoDeadline` invalida anche `["saved-bandi"]`.
