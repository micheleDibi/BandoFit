...
- **`CompatibilitaBadge`** (`components/bandi/`): pill **«Compatibilità 18/23»** — etichetta esplicita + frazione colorata per banda con la stessa scala dell'AI-check (`lib/scoreColor.ts`); percentuale e dettaglio per dimensione nel tooltip/`aria-label`. Presente sulla riga badge di `BandoCard` e nell'header di `BandoDetail`, **subito e senza azioni** dell'utente.
- **`CompatibilitaSection`** (`components/bandi/`, full-width nel dettaglio sotto i meta tile): il **lato del bando** messo a confronto con l'azienda. Frazione grande + barra colorata, poi una riga per dimensione (Regioni, Codici ATECO, Settori, Beneficiari) con **tutte le voci richieste dal bando** e quelle in comune evidenziate in emerald con spunta (`matched_ids`). Casi speciali resi espliciti: badge **«Nazionale»** + nota quando il bando copre tutte le regioni (territorio pieno: evidenziate solo le regioni dove hai una sede), **«non valutato»** quando l'azienda non ha quel dato (la dimensione non entra nel punteggio), e un callout ambra con link ad Azienda quando la P.IVA non è importata. **Assorbe la vecchia card «A chi si rivolge»** (stesse liste, ora con il confronto): in sidebar restano solo le Tematiche. Regioni e Beneficiari occupano l'intera larghezza — sono le liste più lunghe e le due colonne restavano frastagliate.
...
- Il valore arriva già calcolato dal backend nel campo `compatibilita` di `BandoListItem`/`BandoDetail` (tipi `Compatibilita`/`CompatibilitaDimensione` in `types/index.ts`): il frontend non ricalcola nulla e **rende badge e sezione solo se il campo è presente** (profilo aziendale sufficiente, cioè P.IVA importata). Nessun nuovo hook o round-trip: viaggia con la lista e col dettaglio.
